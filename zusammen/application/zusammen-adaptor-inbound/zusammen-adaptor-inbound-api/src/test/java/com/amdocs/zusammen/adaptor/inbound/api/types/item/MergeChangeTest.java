...
public class MergeChangeTest {
  ...
  public void testSetChangedVersion() {
    ...
    Assert.assertEquals(change.getChangedVersion().getAction(), Action.UPDATE);
  }

  @Test
  public void testSetChangedElementsKeepsCallerCollection() {
    MergeChange change = new MergeChange();
    Collection<Element> changedElements = new ArrayList<>();
    ZusammenElement changedElement = new ZusammenElement();
    changedElement.setElementId(new Id("element-1"));
    ...
    Assert.assertEquals(change.getChangedElements().iterator().next().getElementId().getValue(),
        "element-1");
  }
}
