...
public class CollaborationStoreFactoryImplTest {
  ...
  @Test
  public void testCreateInterfaceInstantiatesThePluginConfiguredForTheCollaborativeStore() {
    givenConfiguredPlugin(StubPlugin.class.getName());

    CollaborationStore store =
        new CollaborationStoreFactoryImpl().createInterface(new SessionContext());

    Assert.assertTrue(store instanceof StubPlugin);
  }

  @Test
  public void testCreateInterfaceReusesTheSamePluginInstance() {
    givenConfiguredPlugin(StubPlugin.class.getName());
    CollaborationStoreFactoryImpl factory = new CollaborationStoreFactoryImpl();

    CollaborationStore first = factory.createInterface(new SessionContext());
    CollaborationStore second = new CollaborationStoreFactoryImpl()
        .createInterface(new SessionContext());

    Assert.assertSame(second, first);
  }

  @Test
  public void testCreateInterfaceFailsWhenThePluginClassCannotBeLoaded() {
    givenConfiguredPlugin("com.amdocs.zusammen.sdk.collaboration.MissingPlugin");
    ...
    try {
      ...
      Assert.assertTrue(e.getCause().getMessage().contains("MissingPlugin"));
    }
  }
  ...
  private void givenConfiguredPlugin(String implementationClass) {
    ...
    when(configurationManager.getPluginInfo(SdkConstants.ZUSAMMEN_COLLABORATIVE_STORE))
        .thenReturn(pluginInfo);
  }

  private void clearPluginCache() throws Exception {
    Field field = CollaborationStoreFactoryImpl.class.getDeclaredField("collaborationStore");
