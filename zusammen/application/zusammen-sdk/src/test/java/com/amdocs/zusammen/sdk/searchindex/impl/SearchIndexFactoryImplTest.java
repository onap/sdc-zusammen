...
public class SearchIndexFactoryImplTest {
  ...
  @Test
  public void testCreateInterfaceInstantiatesThePluginConfiguredForTheSearchIndex() {
    givenConfiguredPlugin(StubPlugin.class.getName());

    SearchIndex searchIndex = new SearchIndexFactoryImpl().createInterface(new SessionContext());

    Assert.assertTrue(searchIndex instanceof StubPlugin);
  }
  ...
  public void testCreateInterfaceReusesTheSamePluginInstance() {
    ...
    SearchIndex first = new SearchIndexFactoryImpl().createInterface(new SessionContext());
    SearchIndex second = new SearchIndexFactoryImpl().createInterface(new SessionContext());
  ...
  public void testCreateInterfaceFailsWhenThePluginClassCannotBeLoaded() {
    givenConfiguredPlugin("com.amdocs.zusammen.sdk.searchindex.MissingPlugin");

    try {
      new SearchIndexFactoryImpl().createInterface(new SessionContext());
  ...
  private void givenConfiguredPlugin(String implementationClass) {
    ...
    when(configurationManager.getPluginInfo(SdkConstants.ZUSAMMEN_SEARCH_INDEX))
        .thenReturn(pluginInfo);
  }

  private void clearPluginCache() throws Exception {
    Field field = SearchIndexFactoryImpl.class.getDeclaredField("searchIndex");
