...
public class HealthManagerImplTest {

    private static final SessionContext CONTEXT =
            TestUtils.createSessionContext(new UserInfo("HealthManagerImplTest_user"), "test");
    ...
    private HealthManagerImpl healthManager;
    ...
    @Test
    public void testGetHealthStatusReportsStateThenCollaborationThenSearch() {
        ...
        Collection<HealthInfo> healthInfos = healthManager.getHealthStatus(CONTEXT);

        Assert.assertEquals(new ArrayList<>(healthInfos),
                Arrays.asList(STATE_STATUS, COLLABORATION_STATUS, SEARCH_STATUS));
    }

    @Test
    public void testGetHealthStatusFailsWhenTheHealthAdaptorCannotBeResolved() {
        HealthAdaptorFactoryStubs.installUnresolvableAdaptor();

        ReturnCode returnCode =
                TestUtils.captureFailure(() -> healthManager.getHealthStatus(CONTEXT));
